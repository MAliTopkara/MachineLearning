...
# Ödev Gereksinimleri
# 1. Model class olarak yazılmalı, hazır kütüphaneler kullanılmamalıdır.
# 2. İleri yayılım, geri yayılım, aktivasyon fonksiyonları, gradyan iniş algoritması uygulanmalıdır.
# 3. Gerekli görselleştirmeler (karmaşıklık matrisi, loss-epoch eğrisi) yapılmalıdır.
...
# --- Model Eğitimi ---
print("\n--- Model Eğitimi ---")
input_size = X_train_scaled.shape[1]  # 4 özellik (sepal_length, sepal_width, petal_length, petal_width)
hidden_sizes = [10, 5]  # 2 gizli katman: İlkinde 10, ikincisinde 5 nöron
output_size = len(np.unique(y_train))  # 3 sınıf (Setosa, Versicolor, Virginica)
learning_rate = 0.01
